...
The wire schema is shape-swapped by `task.batch` (default on). One unit of work is the task item `{ name?, agent?, task, model?, role?, effort?, outputSchema?, schemaMode?, isolated? }`. `model` and `role` exist only when `task.perCallModel=true` (default off); `isolated` exists only when `task.isolation.mode` is not `none` **and plan mode is disabled**; `effort` exists only when `task.enableEffort=true` (default off).

- **Batch shape** (`task.batch` on): `{ context, tasks: item[] }` — one subagent per item, all run under the same fan-out rules; there is no top-level agent field. `context` is **required** shared background rendered into every spawned subagent's system prompt (`CONTEXT` section); `agent`, `model`, `role`, `outputSchema`, and `schemaMode` are per item. `model` and `role` are added only when `task.perCallModel` is enabled; `effort` is added only when its setting enables it; `isolated` additionally requires plan mode to be disabled.
...
| `model` | `string \| string[]` | No | Present only with `task.perCallModel=true`. Explicit non-empty model selector or non-empty fallback chain for this spawn. Optional `:reasoning` suffixes are preserved. A literal `"default"` is treated as omitted (inherits from agent frontmatter). Item field in batch shape, top-level in flat shape. |
| `role` | `string` (≤256 chars) | No | Present only with `task.perCallModel=true`. Specialist persona injected into the subagent system prompt. Shapes the subagent's investigation style and decision-making. Item field in batch shape, top-level in flat shape. |
...
7. Model priority: per-call `model` → `task.agentModelOverrides` → agent frontmatter → configured task role/session fallback. A per-call `model: "default"` or `"@default"` is treated as omitted, so agent frontmatter wins. Output schema priority: per-call `outputSchema` → agent frontmatter `output` → inherited parent session schema.
