...
- `agent` defaults from the current spawn policy; optional `model` is a string or ordered fallback list of concrete `provider/model[:thinking]` selectors and `@role` aliases, gated by `task.perCallModel`. `default`/`@default` inherits the chosen agent frontmatter model. The selected agent's frontmatter settings apply; `schema` overrides agent/session schemas and `schemaMode`/`schema_mode` chooses `permissive` or `strict`.
